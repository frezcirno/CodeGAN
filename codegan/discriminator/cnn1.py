...
class CNNClassifier(nn.Module):
    def __init__(
        ...
        hidden_size=768,
        drop_prob=0.1,
        src_max_kernel_size=16,
        tgt_max_kernel_size=16,
    ):
        super().__init__()
    ...
    def forward(self, source_ids: Tensor, target_ids: Tensor):
        ...
        """
        x = self.dropout(self.embedding(source_ids)).unsqueeze(1)
        # [batch_size, chan(1), src_max_len, hidden_size]
        y = self.dropout(self.embedding(target_ids)).unsqueeze(1)
        ...
        # src_tgt = torch.tanh(self.linear(src_tgt))
